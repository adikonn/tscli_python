...
Выбери контест из списка, используя **нумерацию от 1** (как нормальные люди считают, а не программисты 😅).
...
Показывает:
- 📌 Название контеста
- 📝 Список задач (с нумерацией от 1!)
...
- 🎨 Цветная подсветка результатов:
  ...
  - ⚪ Серый = не пытался (`.`)
- 💜 **Твоя команда подсвечена жирным пурпурным** — ты её точно не пропустишь!
- 📏 Компактный формат под 80-колоночный терминал (old school, baby! 🕹️)
- 📊 Показывает: команду, результаты по задачам, количество решённых, время, место
...
**Способ 1: Автоопределение задачи по имени файла** (рекомендуется 👍)
...
- `-l` или `--lang` — номер компилятора от 1 (смотри в `contest show`)

**Способ 3: Интерактивный режим** (для ленивых 😴)
...
Скачивает PDF с условиями задач в текущую директорию. Сохраняется как `statements.pdf`.
...
Выбери компилятор один раз, и потом не нужно будет указывать `-l` каждый раз! Настройка сохраняется в `.tsweb_py.local` (локально для каждой директории).
...
| `.tsweb_py.cookies` | Куки сессии (включая contest_id) 🍪 | `~/.tsweb_py.cookies` |
| `.tsweb_py.local` | Компилятор по умолчанию 🛠️ | Текущая директория |

**Важно:** Не коммить `.tsweb_py.local` и `.tsweb_py.global` в git! Они уже в `.gitignore`. 🙈
...
- ✅ **Нумерация от 1** (как нормальные люди! 🎉)
